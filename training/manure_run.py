...
# Load mô hình cho bón phân
clf = joblib.load("predict_manure.pkl")  # Load the manure prediction model
le = joblib.load("label_encoder.pkl")  # Load the label encoder used during training
...
for i, pred in enumerate(predictions):
    # Convert the numeric prediction back to the corresponding label
    predicted_label = le.inverse_transform([pred])[0]
    print(f"Mẫu {i+1}: {predicted_label}")
